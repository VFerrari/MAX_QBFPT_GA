 ...
 * 
 * @author ccavellucci, fusberti, rsaraiva, sipamplona, vferrari
 * @param <G>
...
public abstract class AbstractGA<G extends Number, F> {
	...
	protected enum populationReplacement{
		ELITE,
		STSTATE;
	}
	...
	/**
	 * Method for population replacement.
	 * Can be elitism or steady state.
	 */
	private final populationReplacement popMethod;
	 ...
	 *            The mutation rate.
	 * @param popMethod
	 * 			  Method for population replacement. Can be elitism or steady state.
	 */
	public AbstractGA(Evaluator<F> objFunction, 
					  Integer generations,
					  Integer popSize,
					  Double mutationRate,
	 ...
	 * and best solution update.
	 *
	 * @param with_div_maint
	 * 			Uses Diversity Maintenance option
	 * @return The best feasible solution obtained throughout all iterations.
	 */
	public Solution<F> solve(double maxTime, Boolean with_div_maint) {
		long startTime = System.currentTimeMillis();
		long endTime;
		double totalTime;
		...
		for (int g = 1; g <= generations; g++) {
			...
			Population offsprings = crossover(parents);
			if(with_div_maint){
				offsprings = crossover(parents, with_div_maint);
			}
	...
	/**
	 * XOR Operation between parent1 and parent2
	 *
	 * @param parent1
	 *            Representing parent1
	 * @param parent2
	 ...
	 *            The selected parents for crossover.
	 * @param with_div_maint
	 * 			  Boolean variable to choose diversity maintenance or not
	 * @return The resulting offsprings.
	 */
	protected Population crossover(Population parents, Boolean with_div_maint) {
		...
		for (int i = 0; i < popSize; i = i + 2) {
			...
			if (with_div_maint) {
				...
				if (possibleCrosspoints.size() >= 2){
					Collections.shuffle(possibleCrosspoints);
	 ...
	 * 
	 * @param original
	 * 			  The original population for the current generation.
	 * @param offsprings
	 ...
	 * @return The updated population for the next generation.
	 */
	private Population elitism(Population offsprings) {
	...
	/**
	 * The method used for updating the population is steady state \lambda + \mu,
	 * which takes the best chromosomes from the set with offsprings and their 
	 ...
	 * @return The updated population for the next generation.
	 */
	private Population steadyState(Population original, Population offsprings) {
		offsprings.addAll(original);
		offsprings.sort(Comparator.comparingDouble(x -> -fitness(x)));
		offsprings.subList(original.size(), offsprings.size()).clear();
		offsprings.trimToSize();
		return offsprings;
	}
